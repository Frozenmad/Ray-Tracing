...
model_list,max_index,min_index = get_model('test.obj',detail=True)
...
camera = Camera(position = arr([1000,500,0]),rotation = arr([[- np.pi / 2.,0.,np.pi]]))

my_space.setCamera(camera)

render = my_space.Render(orth = True, resolution_width = 20, resolution_height = 20, width = 1000, height = 1000)
