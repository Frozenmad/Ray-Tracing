...
def get_model(path, detail = False, texture_d = np.array([1.,1.,1.]), texture_a = np.array([1.,1.,1.]), texture_s = np.array([1.,1.,1.]), ratio_s = 0.5, ratio_d = 0.4, ratio_a = 0.1):
		...
		if 'v ' == line[:2]:
			...
			if detail:
				...
				mod = Complete_Polynominal(ver_list,nver_list,texture_s = np.array([texture_s]*3), 
					texture_d = np.array([texture_d]*3), texture_a = np.array([texture_a]*3), ratio_s = ratio_s, ratio_d = ratio_d, ratio_a = ratio_a)
			else:
				mod = Polynominal(ver_list, texture_s = texture_s, texture_d = texture_d, texture_a = texture_a, ratio_s = ratio_s, ratio_d = ratio_d, ratio_a = ratio_a)
